...
fn main() -> Result<(), slint::PlatformError> {
    ...
    ui.on_utregning_belop(move |total, rente, avdrag, strom1, strom2, strom3| {
        ...
        let avdrag_num: f64 = avdrag.trim().parse::<f64>().unwrap_or(0.0);
        let strom1_num: f64 = strom1.trim().parse::<f64>().unwrap_or(0.0);
        let strom2_num: f64 = strom2.trim().parse::<f64>().unwrap_or(0.0);
        let strom3_num: f64 = strom3.trim()..parse::<f64>().unwrap_or(0.0);
